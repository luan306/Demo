...
namespace API_WebLinkkien.Service
{
    ...
    public class AccountsService : IAccountsService
    {
        ...
        public async Task<Response> GetAllAccounts()
        {
            ...
            if (dt.Rows.Count > 0)
            {
                for (int i = 0; i < dt.Rows.Count; i++)
                {
                    ...
                    account.Password = Convert.ToString(dt.Rows[i]["Password"]);
                    account.Email = Convert.ToString(dt.Rows[i]["Email"]);
        ...
        public async Task<Response>Login(string? email, string password)
        {
            ...
            {
                DataTable dt = new DataTable();
                dt = _connectdata.DataConnect(string.Format("select * from Accounts where Email = N'{0}' and Password = N'{1}'", email, password));
        ...
        public async Task<Response>Register(string email, string password, string fullname, string phone)
        {
           ...
           if(dt.Rows.Count >0)
            ...
            {
                int result = 0;
                result = _connectdata.DataConnectInsert(string.Format("INSERT INTO Accounts(Phone, FullName, Email, Password) values('{0}',N'{1}','{2}','{3}')", phone, fullname, email, password));
